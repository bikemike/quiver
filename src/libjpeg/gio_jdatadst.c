 ...
 * emitting JPEG data to a file (or any gio stream).  While these routines
...
/* Expanded data destination object for gio output */

typedef struct {
  struct jpeg_destination_mgr pub; /* public fields */

  GOutputStream* outfile;		/* target stream */
...
/*
 * Prepare for output to a gio stream.
 * The caller must have already opened the stream, and is responsible
 * for closing it after finishing compression.
 */

GLOBAL(void)
jpeg_gio_dest (j_compress_ptr cinfo, GOutputStream* outfile)
{
  my_dest_ptr dest;

  /* The destination object is made permanent so that multiple JPEG images
   * can be written to the same file without re-executing jpeg_gio_dest.
