...
pkgdatadir = $(datadir)/quiver
pkgincludedir = $(includedir)/quiver
pkglibdir = $(libdir)/quiver
pkglibexecdir = $(libexecdir)/quiver
...
am__installdirs = "$(DESTDIR)$(quiverdocdir)"
DATA = $(quiverdoc_DATA)
...
GREP = /usr/bin/grep
GSTREAMER_CFLAGS = -I/usr/include/gstreamer-1.0 -I/usr/include/glib-2.0 -I/usr/lib/x86_64-linux-gnu/glib-2.0/include -I/usr/include/x86_64-linux-gnu -I/usr/include/orc-0.4 -pthread
GSTREAMER_LIBS = -lgstvideo-1.0 -lgstapp-1.0 -lgstbase-1.0 -lgstreamer-1.0 -lgobject-2.0 -lglib-2.0
...
OTOOL64 =
PACKAGE = quiver
PACKAGE_BUGREPORT = http://mike.yi.org/quiver
...
PACKAGE_NAME = quiver
PACKAGE_STRING = quiver 0.2.0
PACKAGE_TARNAME = quiver
PACKAGE_URL =
...
EXTRA_DIST = config.rpath  ${quiverdoc_DATA} \
     intltool-extract.in \
	 intltool-merge.in \
...
install-quiverdocDATA: $(quiverdoc_DATA)
	@$(NORMAL_INSTALL)
	@list='$(quiverdoc_DATA)'; test -n "$(quiverdocdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(quiverdocdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(quiverdocdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(quiverdocdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(quiverdocdir)" || exit $$?; \
	done

uninstall-quiverdocDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(quiverdoc_DATA)'; test -n "$(quiverdocdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(quiverdocdir)'; $(am__uninstall_files_from_dir)
...
installdirs-am:
	for dir in "$(DESTDIR)$(quiverdocdir)"; do \
...
install-data-am: install-quiverdocDATA
...
uninstall-am: uninstall-quiverdocDATA
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am \
	...
	install-pdf-am install-ps install-ps-am install-quiverdocDATA \
